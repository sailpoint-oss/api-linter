...
export default createOptionalContextRulesetFunction({
    ...
    const versionedPaths = routes.filter(route => route.versionStart !== 0 && route.versionStart != null);
    const nonVersionedPaths = routes.filter(route => route.versionStart === 0 || route.versionStart == null);
    ...
    for (let i = 0; i < routes.length; i++) {
        ...
        if (original.path !== sorted.path) {
            ...
            const currentIdxMap = new Map();
            const expectedIdxMap = new Map();
...
function compareRoutes(route1, route2) {
    ...
    const segments1 = route1.path.split('/').filter(s => s.length > 0);
    const segments2 = route2.path.split('/').filter(s => s.length > 0);
...
function countStaticSegments(segments) {
...
//# sourceMappingURL=check-path-order.js.map